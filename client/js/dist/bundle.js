...
System.registerDynamic("client/js/src/core.js", [], false, function($__require, $__exports, $__module) {
  ...
  (function() {
    ...
    this["getFirstNumberAsPhraseBetween20and90"] = getFirstNumberAsPhraseBetween20and90;
    this["getSecondNumberAsPhraseBetween20and90"] = getSecondNumberAsPhraseBetween20and90;
    this["getFirstNumberAsPhraseOver100"] = getFirstNumberAsPhraseOver100;
    this["sendPhraseToView"] = sendPhraseToView;
    this["firstNineteen"] = firstNineteen;
    this["between20and90"] = between20and90;
    $(document).ready(function() {
      init();
    });
    function init() {
    ...
    function handleConversionToString(integer) {
      ...
      if (integer <= 19) {
        FinalPhrase = firstNineteen(integer);
      } else if (integer >= 20 && integer <= 100) {
        let firstNumberPhrase = getFirstNumberAsPhraseBetween20and90(integer);
        if (integer % 10 === 0) {
          FinalPhrase = firstNumberPhrase;
        } else {
          let secondNumberPhrase = getSecondNumberAsPhraseBetween20and90(integer);
          let combined = firstNumberPhrase + "-" + secondNumberPhrase;
          FinalPhrase = combined;
        }
      } else if (integer >= 101 && integer <= 1000) {
        let firstNumberPhrase = getFirstNumberAsPhraseOver100(integer);
        console.log(firstNumberPhrase);
      }
      sendPhraseToView(FinalPhrase);
    }
    function getFirstNumberAsPhraseBetween20and90(integer) {
      let rootNum = parseInt(integer / 10);
      return between20and90(rootNum);
    }
    function getSecondNumberAsPhraseBetween20and90(integer) {
      let asString = integer.toString();
      let secondNum = asString.substr(1, 2);
      return firstNineteen(secondNum);
    }
    function getFirstNumberAsPhraseOver100(integer) {
      let asString = integer.toString();
      asString = asString.substr(0, 1);
      let asInt = parseInt(asString);
      return firstNineteen(asInt);
    }
    ...
    function between20and90(integer) {
